...
//	return (BYTE)SPIx_DR;		/* Return received byte */
	...
	HAL_SPI_TransmitReceive(&hspi1, &dat, &rxdat, sizeof (BYTE), SDTIMEOUT);
	return (BYTE) rxdat;
}
...
static uint8_t SPI_RxByte(void)
{
	...
	while (HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY);
	HAL_SPI_TransmitReceive(&hspi1, &dummy, &data, 1, SDTIMEOUT);
...
//	SPIx_CR1 |= _BV(6);
	//while(!__HAL_SPI_GET_FLAG(&hspi1, SPI_FLAG_TXE));
	/*while ((HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY));
	HAL_SPI_Receive(&hspi1, buff, btr, btr*SDTIMEOUT);*/
	do {
		SPI_RxBytePtr(buff++);
	} while(btr--);	
}
...
//	SPIx_CR1 |= _BV(6);
	//while(!__HAL_SPI_GET_FLAG(&hspi1, SPI_FLAG_TXE));
	while (HAL_SPI_GetState(&hspi1) != HAL_SPI_STATE_READY);
	HAL_SPI_Transmit(&hspi1, (uint8_t*)buff, btx, btx*SDTIMEOUT);
}
#endif
